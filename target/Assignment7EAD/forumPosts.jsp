...
<head>
    ...
    <style>
        ...
        .blog-author--no-cover h3::before {
            background: url("https://cdn.pixabay.com/photo/2016/08/08/09/17/avatar-1577909_1280.png");
            background-size: cover;
            border-radius: 50%;
            content: " ";
...
<div class="topnav">
    ...
    <c:if test="${user.userLoggedIn == true}">
        <a href="newForumPost.html">Add Post</a>
    </c:if>
    <c:if test="${user.userLoggedIn == true}">
        <a href="logout.jsp">Logout</a>
    </c:if>
    <c:if test="${user.userLoggedIn == false}">
...
<c:forEach var="post"
           items="${ForumPost.findPosts('jdbc:postgresql://localhost:5432/myForum_db?user=postgres&password=1991')}">
    <div class="blog-container">

        <div class="blog-header">
            <div class="blog-author--no-cover">
                <h3><c:out value="${post.user_id}"/></h3>
            </div>
        </div>

        <div class="blog-body">
            <div class="blog-title">
                <h1><a href="#"><c:out value="${post.text}"/></a></h1>
            </div>
            <div class="blog-summary">
                <p><c:out value="${post.subtext}"/></p>
            </div>
            <div class="blog-tags">
                <ul>
                    <li><a href="#">Post</a></li>
        ...
        <div class="blog-footer">
            <ul>
                ...
                <c:if test=
                              "${ForumPost.returnMyLikes('jdbc:postgresql://localhost:5432/myForum_db?user=postgres&password=1991', post.id, user.id != null ? user.id : 0)
                  == true && user.userLoggedIn == true}">
                    <%
                        System.out.println("im here");
                    %>
                    <li class="shares">
                        <form action="likeIt.jsp" method="post">
                            <input name="post_id" type="hidden" value="<c:out value='${post.id}'/>"/>
                            <input name="user_id" type="hidden" value="<c:out value='${user.id}'/>"/>
                            <button class="background-color-transparent">
                                <svg class="icon-star">
                                    <use xlink:href="#icon-star">
                                    </use>
                                </svg>
                                <span class="numero"> <c:out
                                        value="${post.returnPostLikes('jdbc:postgresql://localhost:5432/myForum_db?user=postgres&password=1991', post.id)}"/>
                    </span>
                            </button>
                        </form>
                    </li>
                ...
                <c:if test=
                              "${ForumPost.returnMyLikes('jdbc:postgresql://localhost:5432/myForum_db?user=postgres&password=1991', post.id, user.id != null ? user.id : 0)
                  == false || user.userLoggedIn == false}">
                    <li class="shares">
                        <form action="returnMyLike.jsp" method="post">
                            <input name="post_id" type="hidden" value="<c:out value='${post.id}'/>"/>
                            <input name="user_id" type="hidden" value="<c:out value='${user.id}'/>"/>
                            <button class="background-color-transparent">
                                <svg class="icon-some">
